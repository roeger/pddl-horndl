...
(:derived (AUX0 ?y2 - object ?y1 - object ?y0 - object)
          (and (onblock ?y0 ?y1) (onblock ?y0 ?y2) (not (= ?y1 ?y2))))
(:derived (AUX1 ?y0 - object)
          (and (DATALOG_ONBLOCKSOMETHING ?y0) (DATALOG_ONTABLESOMETHING ?y0)))
(:derived (AUX2 ?y0 - object)
          (and (DATALOG_BLOCK ?y0) (DATALOG_TABLE ?y0)))
(:action pick-up
  :parameters (?x ?y - object)
  :precondition (and (DATALOG_ON ?x ?y) (not (DATALOG_BLOCKED ?x)) (not (DATALOG_INCONSISTENT)) (not (DATALOG_QUERY0)))
  :effect (and (not (on ?x ?y)) (not (onblock ?x ?y)) (not (ontable ?x ?y)) (holding ?x) (not (blocked ?y))))
(:action put-down
  :parameters (?x ?y - object)
