...
(:derived (DATALOG_INCONSISTENT )
          (exists (?y0 - object) (and (DATALOG_SERVICE ?y0) (contract ?y0))))
(:derived (DATALOG_SERVICE ?x0 - object)
...
(:action contractSaving
  :parameters (?x ?y ?z - object)
  :precondition (and (contract ?y) (courierack ?x) (not (DATALOG_INCONSISTENT)))
  :effect (and (crmack ?z)))
(:action courierService
  :parameters (?x ?y - object)
  :precondition (and (contract ?x) (not (DATALOG_INCONSISTENT)))
