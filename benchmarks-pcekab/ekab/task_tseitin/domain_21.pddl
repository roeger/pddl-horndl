...
(:predicates
  ...
  (AUX8 ?x3 - object ?x1 - object ?x2 - object)
  (AUX9 ?y - object ?x - object)
  (AUX10))
...
(:derived (AUX8 ?x3 - object ?x1 - object ?x2 - object)
          (or (= ?x1 ?x2) (= ?x1 ?x3) (= ?x2 ?x3) (not (informaticengineer ?x1)) (not (informaticengineer ?x2)) (not (informaticengineer ?x3))))
(:derived (AUX9 ?y - object ?x - object)
          (and (DATALOG_QUERY0 ?x ?y) (not (= ?x ?y))))
(:derived (AUX10)
          (exists (?x ?y - object) (AUX9 ?y ?x)))
(:action hireElectronicEng
  :parameters (?n - object)
  :precondition (not (DATALOG_INCONSISTENT))
  :effect (when (forall (?x1 ?x2 ?x3 - object) (AUX7 ?x3 ?x1 ?x2)) (electronicengineer ?n)))
(:action hireInformaticEng
  :parameters (?n - object)
  :precondition (not (DATALOG_INCONSISTENT))
  :effect (when (forall (?x1 ?x2 ?x3 - object) (AUX8 ?x3 ?x1 ?x2)) (informaticengineer ?n)))
