...
ARCHIVE_PATH = "roeger/pddl-horn-pcekab/"
REPO = os.environ["DOWNWARD_AIBASEL"]
BENCHMARKS_DIR = project.get_repo_base()/"benchmarks-pcekab/"
...
FULLSUITE.extend(f"cat_2022:problem_{i}.pddl" for i in range(6, 26))
#FULLSUITE.extend(f"catImproved_2025:problem_{i}.pddl" for i in range(6, 26))
FULLSUITE.extend(f"catOG:problem_{i}.pddl" for i in range(6, 26))
#FULLSUITE.extend(f"drones:problem_{i}-{j}.pddl" for i in range(5, 11) for j in range(5,9))
FULLSUITE.extend(f"dronesv2:problem_{i}-{j}.pddl" for i in range(5, 11) for j in range(5,9))
...
FULLSUITE.extend(f"phone_assembly:problem_{i}-0.pddl" for i in range(1, 11))
#FULLSUITE.extend(f"queens:problem_-{i}-{j}.pddl"  for j in range(i-4, i+1) for i in range(5, 11))
FULLSUITE.extend(f"robotConj:problem_{i}.pddl" for i in range(3, 23))
...
REV_NICKS = [
    ("76b1d36e43d119f57aa8e003b51605e1d99350f0", ""), # 11-6-2026
]
ATTRIBUTES = [
    "error",
    "planner_exit_code",
...
benchmark_folders = ["cekab/var0", "cekab/var3", "pcekab/var0", "pcekab/var3",
                     "ekab"]
        ...
        for task in suites.build_suite(BENCHMARKS_DIR, SUITE,
            ...
            for variant, prefix in variants.items():
                for folder in benchmark_folders:
                    ...
                    algo_name = f"{config_nick}-{variant}-{"-".join(folder.split(/)}"
                    #algo_name = f"{config_nick}_{folder}"
                    if rev_nick:
                        #algo_name = f"{rev_nick}-{algo_name}" 
                        algo_name = f"{rev_nick}-{config_nick}-{variant}-{"-".join(folder.split(/)}"
