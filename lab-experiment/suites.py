...
def get_task_variants(benchmarks_dir, domain: str, problem: str, variants, folders):
    result = defaultdict(dict)
    for variant in variants:
        for folder in folders:
            problem_file = Path(benchmarks_dir) / folder / f"{domain}{variant}" / problem
            if not problem_file.exists():
                continue
            domain_file_name = problem.replace("problem", "domain")
            domain_file = Path(benchmarks_dir) / folder / f"{domain}{variant}"/ domain_file_name
            result[folder][variant] = Task(domain, problem,
...
#def _generate_problems(benchmarks_dir, description, variants):
#    """
...
def build_suite(benchmarks_dir, descriptions, variants, folders):
